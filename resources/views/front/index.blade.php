        ...
        <div class="row px-0 px-lg-3 py-2 d-flex align-items-center">
            <div class="col-12 col-lg-9 welcome-user-header">
                <div class="welcome-user-messege">
                    <p class="mb-0 d-none d-md-block">Welcome To <b class="text-warning">Pak Easy Business Survey.</b>
                        Please check your Account Activation Status to get Started <i
                            class="fa-solid fa-arrow-right"></i></p>
                    <p class="d-block d-md-none text-center">Check your Account Status to Start Earn.<i
                            class="fa-solid fa-arrow-down ms-2"></i></p>
                </div>
            </div>
            <div class="col-12 col-lg-3 text-center text-md-end">
                <div class="check-status-btn">
                    <button type="button" class="btn chk-status" data-bs-toggle="collapse"
                        data-bs-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">Check
                        Status</button>
                    <div class="collapse" id="collapseExample">
                        <div class="card card-body">
                            @if (Auth::check() && Auth::user()->admin_approvel_status === 'Approved')
                                <p><b>Your account has been <b
                                            class=" p-1 rounded-2 text-light bg-success">approved</b>. You can now start
                                        earning.</b></p>
                                <?php $url = URL::signedRoute('customer'); ?>
                                <a href="{{ $url }}" class="btn btn-success start-earn-btn mt-2">Start Earning
                                    Now</a>
                            @elseif (Auth::check() && Auth::user()->admin_approvel_status === 'Pending')
                                <p><b>Your account approval is <b
                                            class="text-dark p-1 rounded-2 bg-warning">pending</b>. Please wait for
                                        admin approval.</b></p>
                            @elseif (Auth::check() && Auth::user()->admin_approvel_status === 'Rejected')
                                <p><b>Your account approval request has been <b
                                            class="text-dark p-1 rounded-2 bg-danger">rejected</b>. Please contact for
                                        more information.<a
                                            href="https://chat.whatsapp.com/G1cWIDlc57LEz1ZmEAl6Su">Whatsapp</a></b></p>
                            @else
                                <p><b>You are logged out. Please <a href="{{ route('login') }}">login</a> to
                                        continue.</b></p>
                            @endif
                        </div>
                    </div>
